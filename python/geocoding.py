...
def parse_1e(geo):
    return dict(
        ...
        e_WA1_STREET1_BoroughCode=geo.get(
        ...
        e_WA2_XCoordinate=geo.get(
            'SPATIAL X-Y COORDINATES OF ADDRESS', {}).get('X Coordinate', '',),
        e_WA2_YCoordinate=geo.get(
            'SPATIAL X-Y COORDINATES OF ADDRESS', {}).get('Y Coordinate', '',),
        e_WA2_CommunityDistrict=geo.get(
            'COMMUNITY DISTRICT', {}).get('COMMUNITY DISTRICT', ''),
...
if __name__ == '__main__':
    ...
    df = pd.read_sql('''SELECT DISTINCT id,
                        ...
                        zip as zip_code, 
                        boro 
                        FROM melissa_input;''', BUILD_ENGINE)
    ...
    df = pd.DataFrame(it)
    df.to_csv(Path(__file__).parent.parent /
              'output/melissa_input_geocode.csv', index=False)
    ...
    df.to_sql('melissa_corrections_geocode', BUILD_ENGINE,
